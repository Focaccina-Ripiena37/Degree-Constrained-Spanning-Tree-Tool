...
class App:
    ...
    def show_critical_warning(self, message):
        """Mostra un avviso critico all'utente."""
        messagebox.showwarning("Avviso Critico", message)

    def show_critical_confirmation(self, message, field_name, current_value, safe_value):
        """
        Mostra un avviso critico con opzioni di conferma all'utente.

        Args:
            message: Messaggio di avviso da mostrare
            field_name: Nome del campo che ha il valore critico
            current_value: Valore attuale (critico)
            safe_value: Valore sicuro da ripristinare se l'utente cancella

        Returns:
            bool: True se l'utente sceglie di procedere, False se cancella
        """
        # Aggiungi opzioni di conferma al messaggio
        confirmation_message = message + "\n\n" + (
            "🤔 COSA VUOI FARE?\n\n"
            "• PROCEDI: Continua con i valori attuali (a tuo rischio)\n"
            "• ANNULLA: Ripristina valori sicuri e modifica i parametri\n\n"
            "⚠️ IMPORTANTE: Se procedi, accetti la responsabilità\n"
            "per eventuali problemi di performance o stabilità del sistema."
        )

        # Mostra dialog di conferma
        result = messagebox.askyesno(
            "⚠️ Conferma Parametri Critici",
            confirmation_message,
            icon='warning'
        )

        if result:
            # L'utente ha scelto "Sì" (Procedi)
            self.log_message(f"⚠️ ATTENZIONE: L'utente ha confermato l'uso di parametri critici: {field_name}={current_value}", "warning")
            return True
        else:
            # L'utente ha scelto "No" (Annulla) - ripristina valore sicuro
            self.revert_field_to_safe_value(field_name, safe_value)
            self.log_message(f"✅ Parametro {field_name} ripristinato al valore sicuro: {safe_value}", "info")
            return False

    def revert_field_to_safe_value(self, field_name, safe_value):
        """
        Ripristina un campo al suo valore sicuro.

        Args:
            field_name: Nome del campo da ripristinare
            safe_value: Valore sicuro da impostare
        """
        try:
            ...
            # Mostra messaggio di conferma del ripristino
            messagebox.showinfo(
                "Valore Ripristinato",
                f"Il parametro '{field_name}' è stato ripristinato al valore sicuro: {safe_value}\n\n"
                "Puoi ora modificare i parametri con valori più sicuri prima di avviare il calcolo."
            )

        except Exception as e:
            self.log_message(f"Errore nel ripristino del campo {field_name}: {e}", "error")
    ...
    def validate_inputs(self):
        ...
        # Controlli per avvisi critici sui parametri con conferma utente
        if not self.check_critical_parameters():
            return False  # L'utente ha cancellato a causa di parametri critici

        return True

    def check_critical_parameters(self):
        """
        Controlla i parametri critici e mostra avvisi con conferma utente.

        Returns:
            bool: True se tutti i parametri sono accettabili o confermati dall'utente, False altrimenti
        """
        # 1. Controllo CRITICO per grafo di grandi dimensioni
        n_large = int(self.n_large.get())
        if n_large >= 1000:
            warning_msg = (
                "🔴 RISCHIO ELEVATO DI CRASH DEL SISTEMA!\n\n"
                f"L'istanza grande ha {n_large} nodi (≥ 1000).\n"
                "QUESTO PUÒ CAUSARE:\n"
                "• CRASH CRITICO DEL SISTEMA (0xc000012d)\n"
                "• BLOCCO COMPLETO DEL PC\n"
                "• PERDITA DI DATI NON SALVATI\n"
                "• RIAVVIO FORZATO DEL SISTEMA\n\n"
                "🛑 RACCOMANDAZIONE FORTE:\n"
                "Ridurre a meno di 800 nodi per evitare crash.\n"
                "Il sistema è configurato in SAFE MODE ma grafi\n"
                "di queste dimensioni rimangono pericolosi."
            )
            if not self.show_critical_confirmation(warning_msg, "n_large", n_large, 500):
                return False  # L'utente ha cancellato
        elif n_large >= 800:
            warning_msg = (
                "⚠️ ATTENZIONE: Grafo di dimensioni critiche!\n\n"
                f"L'istanza grande ha {n_large} nodi (≥ 800).\n"
                "Questo potrebbe causare:\n"
                "• Tempi di esecuzione molto lunghi (>30 min)\n"
                "• Elevato consumo di memoria (>8GB)\n"
                "• Possibili rallentamenti del sistema\n"
                "• Timeout automatici degli algoritmi\n\n"
                "🛡️ Il sistema è in SAFE MODE ma si consiglia\n"
                "di monitorare attentamente le prestazioni."
            )
            if not self.show_critical_confirmation(warning_msg, "n_large", n_large, 500):
                return False  # L'utente ha cancellato
        elif n_large >= 500:
            warning_msg = (
                "⚠️ Grafo di grandi dimensioni rilevato!\n\n"
                f"L'istanza grande ha {n_large} nodi (≥ 500).\n"
                "Questo potrebbe causare:\n"
                "• Tempi di esecuzione lunghi (10-20 min)\n"
                "• Consumo di memoria elevato (4-8GB)\n"
                "• Parallelizzazione automaticamente disabilitata\n\n"
                "ℹ️ Il sistema userà algoritmi sequenziali per sicurezza.\n"
                "IMPATTO SULLE PRESTAZIONI:\n"
                "• Uso intensivo della CPU per 10-20 minuti\n"
                "• Consumo di memoria fino a 4-8GB\n"
                "• Possibili rallentamenti di altre applicazioni"
            )
            if not self.show_critical_confirmation(warning_msg, "n_large", n_large, 200):
                return False  # L'utente ha cancellato

        # 2. Controllo per bassa connettività
        p_val = float(self.p_val.get())
        if p_val < 0.1:
            warning_msg = (
                "⚠️ Bassa connettività del grafo rilevata!\n\n"
                f"Il grado di connessione è {p_val:.2f} (< 0.1).\n"
                "Questo potrebbe causare:\n"
                "• Grafo disconnesso o poco connesso\n"
                "• Fallimenti degli algoritmi\n"
                "• Rallentamenti significativi\n"
                "• Soluzioni di qualità scadente\n\n"
                "IMPATTO SULLE PRESTAZIONI:\n"
                "• Algoritmi potrebbero non trovare soluzioni valide\n"
                "• Tempi di esecuzione imprevedibili\n"
                "• Possibili errori durante l'ottimizzazione\n"
                "• Risultati non rappresentativi"
            )
            if not self.show_critical_confirmation(warning_msg, "p_val", p_val, 0.3):
                return False  # L'utente ha cancellato

        # 3. Controllo per penalità bassa
        penalty = int(self.penalty.get())
        if penalty < 100:
            warning_msg = (
                "⚠️ Valore di penalità basso rilevato!\n\n"
                f"La penalità è {penalty} (< 100).\n"
                "Questo potrebbe causare:\n"
                "• Distorsione dei costi dell'albero\n"
                "• Difficoltà nel confronto delle soluzioni\n"
                "• Risultati poco significativi\n"
                "• Violazioni del vincolo di grado non penalizzate adeguatamente\n\n"
                "IMPATTO SULLA QUALITÀ DEI RISULTATI:\n"
                "• I vincoli di grado potrebbero essere ignorati\n"
                "• Confronti tra algoritmi non significativi\n"
                "• Soluzioni non conformi ai requisiti del problema\n"
                "• Metriche di valutazione distorte"
            )
            if not self.show_critical_confirmation(warning_msg, "penalty", penalty, 1000):
                return False  # L'utente ha cancellato

        # Tutti i controlli sono passati o confermati dall'utente
        return True
    ...
    def run_optimization(self):
        ...
        try:
            ...
            plot_dir = get_current_plot_directory()
            self.queue.put(("log", (f"📁 Risultati verranno salvati in: {plot_dir}", "info")))
        except Exception as e:
            import traceback
            traceback.print_exc()
            return
        ...
        try:
            ...
            for instance_name, n_nodes in instances.items():
                ...
                try:
                    ...
                    import traceback
                    traceback.print_exc()
                    self.queue.put(("error", f"Errore nella generazione del grafo: {e}"))
                    return
                ...
                try:
                    ...
                    import traceback
                    traceback.print_exc()
                    self.queue.put(("error", f"Errore nell'ottimizzazione per {instance_name}: {e}"))
                    return
    ...
    def save_results(self, instances, max_children, penalty, results, plot_dir):
        ...
        try:
            ...
            self.queue.put(("status", "Preparazione batch generazione immagini..."))
            self.queue.put(("log", ("🚀 Utilizzo generazione immagini parallela per migliorare le prestazioni", "info")))
            ...
            for size_name, instance_results in results.items():
                ...
                if "graph" in instance_results:
                    ...
                    image_descriptions.append(f"Grafo {dim_label} ({size_name})")
                ...
                for algo in ["greedy", "local", "sa"]:
                    ...
                    if tree_key in instance_results:
                        ...
                        image_descriptions.append(f"Albero {algo_name[algo]} {dim_label} ({size_name})")

            # Execute batch image generation with progress tracking
            if image_tasks:
                self.queue.put(("log", (f"🎨 Generazione di {len(image_tasks)} immagini in parallelo...", "info")))
                ...
                def image_progress_callback(completed, total):
                    ...
                    if completed <= len(image_descriptions):
                        desc = image_descriptions[completed-1] if completed > 0 else "Immagine"
                        self.queue.put(("status", f"Completata: {desc} ({completed}/{total})"))
                        self.queue.put(("log", (f"✅ {desc} generata ({completed}/{total})", "success")))
                ...
                self.queue.put(("log", (f"🎉 Generazione immagini completata: {successful_images}/{len(image_tasks)} successi", "success")))
            ...
            if table_data:
                ...
                self.queue.put(("status", "Generazione tabella comparativa..."))
                ...
                try:
                    ...
                    if table_result:
                        ...
                        self.queue.put(("log", ("⚠️ Errore nella generazione della tabella", "warning")))
                except Exception as e:
                    self.queue.put(("log", (f"❌ Errore nella generazione tabella: {e}", "error")))
